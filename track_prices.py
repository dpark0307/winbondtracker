...
# ---------------------------------------------------------------------
FIELDNAMES = ["date", "density", "type", "manufacturer", "part_number", "source", "price_usd", "pct_change_vs_last_pull"]


def load_last_prices():
    """
    Reads prices.csv to find each (part_number, source) pair's most
    recent saved price. Keying by source too, since the same part
    number can now have a separate DigiKey price and Mouser price -
    they should be compared independently, not against each other.
    ...
    with open(CSV_PATH, newline="") as f:
        for row in csv.DictReader(f):
            ...
            source = row.get("source", "DigiKey")  # older rows won't have this column
            price = row.get("price_usd")
            if not part or price is None or price == "":
                continue
            try:
                last[(part, source)] = float(price)
            except ValueError:
                continue
    return last
...
def main():
    ...
    if not mouser_key:
        print("NOTE: MOUSER_API_KEY not set - will only pull DigiKey prices this run.")
    ...
    sources_per_part = 2 if mouser_key else 1
    print(f"\nPulling prices for {len(active_parts)} part(s) x {sources_per_part} source(s) on {today}:\n")
    ...
    for density, ptype, manufacturer, part in PARTS:
        ...
        # Build the list of (source_name, fetch_function) to try for this part
        sources_to_try = [("DigiKey", lambda p: fetch_digikey_price(p, digikey_token, digikey_id))]
        if mouser_key:
            sources_to_try.append(("Mouser", lambda p: fetch_mouser_price(p, mouser_key)))

        for source_name, fetch_fn in sources_to_try:
            price, error = fetch_fn(part)

            if error:
                print(f"    [{source_name:<8}] {manufacturer:<12} {part:<22} FAILED - {error}")
                continue

            key = (part, source_name)
            ...
            if prev is not None and prev != 0:
                ...
                print(f"    [{source_name:<8}] {manufacturer:<12} {part:<22} ${price:.4f}  ({pct_change:+.2f}% vs last pull){flag}")
            else:
                print(f"    [{source_name:<8}] {manufacturer:<12} {part:<22} ${price:.4f}  (first entry)")
...
#   setx MOUSER_API_KEY "your-mouser-api-key-here"
#
# MOUSER_API_KEY is optional - if not set, the script still runs and
# just pulls DigiKey prices only, same as before.
#
